...
contract("Proxy", (accounts) => {
    ...
    describe('Loyalty Contract Behavior', function () {
        ...
        describe('Register a member', function () {
            ...
            it('success', async function () {
                const result = await loyalty.methods['registerMember()'].sendTransaction({ from: accounts[1] })
                assert.equal('0x01', result.receipt.status, 'register member')
            })

            it('revert', async function () {
                await assertRevert(loyalty.methods['registerMember()'].sendTransaction({ from: accounts[1] }))
            })
